...
- **Latest full run (current model):** [`eval/run3-full-rerun.md`](eval/run3-full-rerun.md) scored 16 of 22 clean, 3 partial, 3 disagreements with unverified reference answers, and 0 clear errors; all four fee questions matched the official GOV.UK figures. It also showed the Copilot attributing specific claims to "official guidance" that could not be supported, and disagreeing with the Visa Search checklist on three points. Each question was asked once, so treat it as a snapshot.
